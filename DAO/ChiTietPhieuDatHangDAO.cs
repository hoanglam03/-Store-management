    ...
    public class ChiTietPhieuDatHangDAO
    {
        ...
        public static DataTable Load_DSCTPDH()
        {
            try
            {
                ...
                string select = "SELECT id,ma_pdh,chi_tiet_pdh.ma_sp,db_sanpham.ten_sp,chi_tiet_pdh.soluong,db_sanpham.gia_sp,thanhtien FROM chi_tiet_pdh inner join db_sanpham on chi_tiet_pdh.ma_sp = db_sanpham.ma_sp";
        ...
        public static DataTable Load_DSTheoMaPDH(PhieuDatHangDTO pdh)
        {
            try
            {
                DataTable dt = new DataTable();
                cnn = DataProvider.ConnectData();
                string select = string.Format("SELECT id,ma_pdh,chi_tiet_pdh.ma_sp,db_sanpham.ten_sp,chi_tiet_pdh.soluong,db_sanpham.gia_sp,thanhtien FROM chi_tiet_pdh inner join db_sanpham on chi_tiet_pdh.ma_sp = db_sanpham.ma_sp WHERE ma_pdh='{0}'", pdh.ma_pdh);
                dt = DataProvider.Load_database(select, cnn);
                cnn.Close();
                return dt;
            }
        ...
        public static void delete_SPPDH(SanPhamDTO sp)
        {
            ...
            {
                cnn = DataProvider.ConnectData();
                string id = string.Format("SELECT id FROM chi_tiet_pdh WHERE ma_sp='{0}';",sp.ma_sp);
                DataTable dt = new DataTable();
                ...
                if (dt != null)
                {
                    ...
                    {
                        DataRow id2 = dt.Rows[i];
                        DataProvider.Execute(cnn, string.Format("delete FROM chi_tiet_pdh WHERE id = '{0}';", id2[0].ToString()));
                    }
                }
                cnn.Close();
            }
