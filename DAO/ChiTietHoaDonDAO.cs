    ...
    public class ChiTietHoaDonDAO
    {
        ...
        public static DataTable Load_DSCTHD()
        {
            try
            {
                ...
                string select = "SELECT id,ma_hd,db_sanpham.ma_sp,db_sanpham.ten_sp,so_luong,db_sanpham.gia_sp,thanh_tien FROM chi_tiet_hd inner join db_sanpham on chi_tiet_hd.ma_sp = db_sanpham.ma_sp";
        ...
        public static DataTable Load_DSTheoMaHD(HoaDonDTO hd)
        {
            try
            {
                DataTable dt = new DataTable();
                cnn = DataProvider.ConnectData();
                string select = string.Format("SELECT id,ma_hd,db_sanpham.ma_sp,db_sanpham.ten_sp,so_luong,db_sanpham.gia_sp,thanh_tien FROM chi_tiet_hd inner join db_sanpham on chi_tiet_hd.ma_sp = db_sanpham.ma_sp WHERE chi_tiet_hd.ma_hd = '{0}'", hd.ma_hd);
                dt = DataProvider.Load_database(select, cnn);
                cnn.Close();
                return dt;
            }
        ...
        public static void delete_SPHD(SanPhamDTO sp)
        {
            ...
            {
                cnn = DataProvider.ConnectData();
                string id = string.Format("SELECT id FROM chi_tiet_hd WHERE ma_sp='{0}';", sp.ma_sp);
                DataTable dt = new DataTable();
                ...
                if (dt != null)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        DataProvider.Execute(cnn, string.Format("delete FROM chi_tiet_hd WHERE id = '{0}';", id2[0].ToString()));
                    }
                }
                cnn.Close();
            }
